...
def generate_certificates(names, font_size, font_path, certificate_path, only_preview=False):
    ...
    if only_preview:
        ...
        img = Image.open(f"generate/certificate{1}.jpg")
        ...
        # with (total=len(names), desc="Generating Certificates":
            for index,name in enumerate(names):
                name = name.strip().title()
                gc.generate_certificate(index, name, len(names))
                ...
                time.sleep(0.1)  # Add a small delay to visualize the progress
                # pbar.update(1)
        ...
        try:
            ...
            if st.button("Generate and Download Certificates"):
                ...
                if font_path is None or certificate_path is None:
                    ...
                    zip_file = zipfile.ZipFile("generate/certificates.zip", "w")
                    ...
                    with open("generate/certificates.zip", "rb") as f:
                        zip_data = f.read()
                        b64_zip_data = base64.b64encode(zip_data).decode("utf-8")
                        href = f'<a href="data:application/zip;base64,{b64_zip_data}" download="generate/certificates.zip">Download Certificates</a>'
                        st.download_button("Download Certificates", data=zip_data, file_name="generate/certificates.zip")
